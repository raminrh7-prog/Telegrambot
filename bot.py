...
# ---------- تابع کمکی ساخت کیبورد پست (جدید) ----------
def build_dynamic_keyboard(weight, work, profit):
    keyboard_btns = []
...
def schedule_post_with_timer(bot, post_data):
    ...
    def send_scheduled_post():
        p = post_data["post"]
        mode = post_data["mode"]
        keyboard = build_dynamic_keyboard(p['weight'], p['work'], p['profit'])
...
def start(update: Update, context: CallbackContext):
    ...
    if update.message:
        update.message.reply_text("به پنل مدیریت ربات خوش آمدید 👇", reply_markup=main_menu())
    else:
        update.callback_query.message.reply_text("به پنل مدیریت ربات خوش آمدید 👇", reply_markup=main_menu())
...
def menu_button(update: Update, context: CallbackContext):
    ...
    if query.data == "settings_main":
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("⬅️ بازگشت", callback_data="back_to_main_from_settings")]
        ])
        query.message.edit_text("⚙️ تنظیمات نمایش کلیدها زیر پست‌ها:", reply_markup=keyboard)
...
def settings_handler(update: Update, context: CallbackContext):
    ...
    if data == "back_to_main_from_settings":
    ...
    if data.startswith("set_btn_"):
        ...
        query.message.edit_text(f"وضعیت کلید {btn_num}:", reply_markup=keyboard)
        return SETTINGS_STATE
    ...
    if data.startswith("on_") or data.startswith("off_"):
        ...
        if not status:
            ...
            query.message.edit_text(f"✅ کلید {btn_num} خاموش شد.")
            start(update, context)
            return ConversationHandler.END
        else:
            if btn_num in ["1", "2"]:
                bot_settings[key] = True
                query.message.edit_text(f"✅ کلید {btn_num} روشن شد.")
                start(update, context)
                return ConversationHandler.END
            else:
                context.user_data["editing_btn"] = btn_num
                query.message.edit_text("🔗 نام و لینک را بفرستید:\nButton - http://link.com", 
                                      reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ بازگشت", callback_data=f"set_btn_{btn_num}")]]))
                return SET_LINK
...
def save_link_handler(update: Update, context: CallbackContext):
    ...
    if " - " in text:
        try:
            ...
            update.message.reply_text(f"✅ کلید {btn_num} تنظیم شد.")
            start(update, context)
            return ConversationHandler.END
        except:
            update.message.reply_text("❌ خطا در فرمت.")
            return SET_LINK
    return SET_LINK
...
def send_post(bot, post, mode):
    keyboard = build_dynamic_keyboard(post['weight'], post['work'], post['profit'])
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            SET_LINK: [MessageHandler(Filters.text & ~Filters.command, save_link_handler), CallbackQueryHandler(settings_handler)]
        },
